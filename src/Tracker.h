...
cv::Mat drawRect(cv::Mat img, cv::Scalar color, int thickness, cv::Vec2f pt1, cv::Vec2f pt2, cv::Vec2f pt3, cv::Vec2f pt4);
...
cv::Vec2i project(cv::Mat H, cv::Vec2f p);

std::vector<cv::Point2f> project(cv::Mat H, std::vector<cv::Vec2f> vecs);
...
private:
	std::vector<TrackRect> init();
	std::vector<cv::Mat> track(std::vector<TrackRect> t);
